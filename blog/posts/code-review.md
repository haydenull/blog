...
3. 功能分支开发完毕后，向需求分支发起 Merge Request（MR）。在发起 MR 时，应提供以下信息：
...
### 通知 Reviewer
在发起 MR 后，使用即时通讯工具（如企业内部使用的 Popo）通知 Reviewer。Reviewer 可以将聊天记录设为待办事项，以便于跟踪。

### Reviewer 的工作
收到通知后，Reviewer 必须在 1 个工作日内完成 Review。理想情况下，应尽快处理，除非有其他紧急任务。为了确保 Review 过程的高效，我们需要注意以下几点：
...
1. **复杂的需求需要先进行方案设计，并邀请你的 reviewer 先沟通一次**：这可以避免在开发四五天后出现大问题，迫于工期只能寄希望于很可能不会有的重构。
2. **针对复杂或重要的逻辑加测试**：可以先从检测 JavaScript 和 TypeScript 方法开始，逐步养成这个习惯。
...
5. **联调及测试期间一般 MR 都不大且代码比较容易 review**：不用担心修复 bug 速度不及时。
...
9. **代码是给人看的而不是机器**：我们要编写易于理解和维护的代码，以便其他团队成员可以更好地进行 Code Review 和后期维护。
