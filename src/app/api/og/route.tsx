...
export async function GET(req: Request) {
  ...
  return new ImageResponse(
    (
      <div
        tw="flex flex-col w-full h-full px-[40px] py-[40px] justify-between"
        style={{
      ...
      >
        <div tw="flex items-center">
          <div tw="flex w-[60px] h-[60px] rounded-full shadow overflow-hidden mr-[20px]">
            {/* @ts-expect-error src 的类型正确 */}
            <img tw="w-full h-full" src={avatarImg} />
          </div>
          <span tw="text-[24px] text-zinc-700">启封 Hayden</span>
        </div>
        <div tw="flex flex-col text-left">
          <h1 tw="text-left text-[52px] font-bold m-0">{title}</h1>
          {description ? <p tw="text-left text-[40px] text-zinc-500 m-0">{description}</p> : null}
        </div>
        <div tw="flex justify-end">
          <span tw="text-[18px] text-violet-500">HaydenHayden.com</span>
        </div>
      </div>
    ),
    {
